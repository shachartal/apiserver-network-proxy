...
import (
	...
	"sigs.k8s.io/apiserver-network-proxy/cmd/bucket-server/app"
	"sigs.k8s.io/apiserver-network-proxy/cmd/bucket-server/app/options"
	"sigs.k8s.io/apiserver-network-proxy/pkg/util"
)

func main() {
	proxy := &app.BucketProxyServer{}
	o := options.NewBucketProxyServerOptions()
	command := app.NewBucketServerCommand(proxy, o)
	flags := command.Flags()
