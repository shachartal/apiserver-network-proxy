...
import (
	...
	"sigs.k8s.io/apiserver-network-proxy/cmd/bucket-agent/app"
	"sigs.k8s.io/apiserver-network-proxy/cmd/bucket-agent/app/options"
	"sigs.k8s.io/apiserver-network-proxy/pkg/util"
)

func main() {
	agent := &app.BucketProxyAgent{}
	o := options.NewBucketProxyAgentOptions()
	command := app.NewBucketAgentCommand(agent, o)
	flags := command.Flags()
